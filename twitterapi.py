...
#grab user tweets
def user_tweets(user, name, state, tweet_num=0):
	all_tweets = []
	api = authenticate()
	try:
		newest_tweets = api.user_timeline(screen_name=user, count = tweet_num)
	except:
		print("No user found")
		return
	all_tweets.extend(newest_tweets)
	i = 1
	to_print = ["Governor: " + name + "\n"]
	to_print += ["Tweets from: @" + user + "\n"]
	for status in all_tweets:
		to_print += [str(i)+ ". "+ status.text +"\n"]
		i+=1
	write(to_print, state)
